...
export default function Create() {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      const prompt = `Transform the provided photo of this ${breed} ${speciesLabel} named "${safeName}" into the following artistic style. CRITICALLY IMPORTANT: Preserve the ${speciesLabel}'s EXACT coat color pattern — including where each color appears on the body (lighter vs darker areas, two-tone patterns, patches, markings). Do NOT simplify into one uniform color. Keep all of the ${speciesLabel}'s actual appearance and features recognizable. Pay special attention to any ASYMMETRIC or UNUSUAL features (e.g., one ear up and one down, a permanently flipped ear, a crooked tail, an underbite, heterochromia) — these quirks are what make this ${speciesLabel} unique and MUST be preserved exactly as shown in the photo. Do NOT "correct" or normalize these features to match breed standard.${realismNote} Style: ${selectedStyle!.promptTemplate.replace("{breed}", breed)}`;
